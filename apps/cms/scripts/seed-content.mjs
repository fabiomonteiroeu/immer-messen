...
const buildCaseSections = (def, locale) => {
  ...
  // Hoje os dois cases tem `sections` proprias nos tres locales, entao o fallback
  // abaixo nao dispara. Ele fica como rede para um case novo que entre so em pt-BR:
  // herda a estrutura do pt-BR (corpo em portugues) em vez de degradar para um
  // paragrafo corrido mais as figuras soltas. Mesmo nesse caso, tudo que ja existe
  // traduzido vem do locale: hero, desafio, lead, alt e legenda das figuras, slug e SEO.
...
const caseStudyDefs = [
  {
    ...
    en: {
      ...
      sections: [
        { __component: "case.text-section", body: "<p>Petrobras carries out continuous monitoring of marine species populations across its operating areas, seeking to understand and mitigate any environmental impact of its activities. As part of that commitment, the company is one of the largest sponsors of marine biodiversity protection programmes in Brazil. One of the main focuses of this effort is the underwater acoustic environment: cetaceans — whales and dolphins — depend on sound to communicate, navigate and breed, which makes them particularly sensitive to noise of human origin, such as that generated by seismic survey campaigns.</p><p>To track that impact, Petrobras works with Instituto Aqualie, one of the world's leading authorities in marine bioacoustics and responsible for one of the longest-running passive acoustic monitoring programmes in the South Atlantic. The Institute combines different technologies — hydrophones, acoustic tags, visual sightings and population tracking — to map the presence, behaviour and seasonal distribution of cetaceans along the Brazilian coast.</p>" },
        { __component: "case.highlight-section", variant: "opening", eyebrow: "The solution", heading: "Optical fibre as an acoustic sensor", body: "<p><b>Since 2024, Instituto Aqualie has contracted Immer Messen to develop an approach that is the first of its kind in Brazil:</b> turning submarine telecom cables already installed by Petrobras into continuous acoustic sensors, through Distributed Fibre Optic Sensing (DFOS) technology, specifically in its DAS (Distributed Acoustic Sensing) mode.</p>" },
        { __component: "case.text-section", body: "<p>The principle is straightforward: idle optical fibres inside a telecom cable — ones that carry no data traffic — can be interrogated by equipment installed onshore, capable of detecting nanoscale strain along the entire length of the cable. Every metre of fibre effectively becomes a listening point. No physical intervention in the existing subsea infrastructure is required.</p>" },
        { figureKey: "mapa-campos" },
        { __component: "case.text-section", body: "<p>The project also draws, in a complementary way, on historical Permanent Reservoir Monitoring (PRM) data — seismic sensors originally installed by Petrobras at the Jubarte Field for reservoir monitoring — repurposed by Instituto Aqualie to confirm the seasonal occurrence of humpback, fin and sei whales in the Campos Basin.</p><p>After a laboratory testing phase in 2024 — which validated the system's sensitivity to low-frequency sound and calibrated parameters such as cable tension and gauge length — Immer Messen installed its interrogation system at a telecom station in Barra do Furado (RJ), monitoring a submarine cable more than <b>110 km long</b> in the Campos Basin.</p>" },
        { __component: "case.section-title", title: "Artificial intelligence in anomaly detection" },
        { __component: "case.two-column-section", leftBody: "<p>The volume of data generated by a DAS system operating continuously over a hundred kilometres of cable is on the order of tens of terabytes per campaign — impossible to inspect manually. To handle that scale, Immer Messen has been developing, since 2020, its own automatic anomaly detection algorithm, based on analysing the power spectral density (PSD) of the signal at each point of the cable across defined time windows. The algorithm sweeps the dataset in space and time, identifying abrupt energy variations in specific frequency bands and generating a catalogue of events — each associated with a position, time and spectral signature — later validated by specialists at Instituto Aqualie.</p>", pullQuote: "today, monitoring works in real time", rightBody: "<p>This layer of artificial intelligence is what makes the system operationally viable: it lets biologists at Instituto Aqualie, from their headquarters in Juiz de Fora, Minas Gerais, remotely track the presence of whales around the Petrobras telecom cables — with no need for vessels, field teams or travel to the coast.</p>" },
        { figureKey: "espectrograma" },
        { __component: "case.panel-section", icon: "bar_chart", defaultOpen: true, title: "Results", body: "<p>The field campaigns confirmed the viability of the technology for bioacoustics at real scale. In a measurement taken about <b>21 km from the coast</b>, the system identified 17 distinct vocalisations within a window of only 90 seconds, with spectral patterns in the <b>40 to 120 Hz</b> range — consistent with fin whale vocalisations. The signals appear in the data as characteristic &quot;V&quot;-shaped patterns, a result of the acoustic wave propagating along the cable, which makes it possible not only to detect the sound but also to locate its origin to within a few metres.</p>", blocks: [
          { heading: "Vessels", body: "<p>The same infrastructure and the same detection pipeline proved sensitive to other classes of signal relevant to offshore operations. In the 30 to 37 km stretch (about 33.5 km from the coast), the system recorded sequential hyperbolic patterns characteristic of vessel noise. The attenuation observed near the apex of each pattern relates to the geometry between the vessel's movement and the cable: intensity rises as the vessel approaches the fibre and falls as it moves away. The temporal evolution of these arrivals makes it possible to infer the direction of travel and, combined with AIS data, to estimate the vessel's trajectory.</p>", figureKey: "embarcacoes" },
          { heading: "Ocean processes and the surf zone", body: "<p>In the coastal section, where the cable enters the sea, the same dataset clearly captured large-scale ocean processes. With a high-pass filter at 0.01 Hz, slow periodic patterns became evident, travelling from the sea towards the shore and attributed to the modulation of hydrostatic pressure by surface gravity waves over the buried fibre. Adjusting the cut-off to 5 Hz, these patterns end abruptly at a specific spatial transition, beyond which higher-frequency mechanical oscillations emerge, associated with surf dynamics at the sand–sea interface. This spectrally and spatially broad response — from low-frequency ocean processes to higher-frequency acoustic emissions — shows the versatility of DAS as an environmental sensor.</p>", figureKey: "processos-oceanicos" },
          { body: "<p>This characteristic is central to the value of the solution — the specificity of the detection lies in the software, not the hardware, which makes it possible to calibrate the same physical system for different monitoring purposes.</p>" },
        ] },
        { __component: "case.section-title", title: "Impact and next steps" },
        { __component: "case.text-section", body: "<p>The results should contribute directly to the scientific understanding of cetacean migration patterns along the Brazilian coast. Species such as the humpback whale use the corridor between the South Atlantic and the north-eastern coast during the breeding season, and continuous data like that generated by the DAS system will help Instituto Aqualie refine population estimates and identify which species pass through different stretches of the coast over the year — essential information for planning seismic campaigns with lower environmental impact.</p><p>The partnership between Immer Messen, Instituto Aqualie and Petrobras is still developing, with the goal of expanding the geographic coverage of the monitoring and improving automatic species classification models over the coming years.</p>" },
        { __component: "case.highlight-section", variant: "closing", heading: "The project is recognised as the first DAS whale monitoring carried out in the South Atlantic", body: "<p>This establishes Immer Messen as a <b>reference in distributed acoustic monitoring solutions</b> for offshore environments — a capability that extends, on the same technological base, to applications such as subsea asset security, intrusion detection on pipelines and cables, and structural monitoring of critical infrastructure.</p>" },
        { __component: "case.text-section", body: "<p>The results described in this case are documented in a peer-reviewed, open-access book chapter:</p><p>ANDRIOLO, A.; MARCON, E. H.; DE CASTRO, F. R.; RODRIGUES, G. M.; MIRANDA, G. A.; VIANA, Y.; GOLODNE, P. M.; POEYS, R. A. C.; SILVA, A. A. C.; AMORIM, T. O. S.; DJOKIC, D.; PIZZORNO, J. L. A. Permanent Reservoir Monitoring (PRM) and Distributed Acoustic Sensing (DAS) as Advanced Technologies for Maximizing the Acquisition of Acoustic Environmental Information. In: POPPER, A. N. et al. (eds.). <i>The Effects of Noise on Aquatic Life IV</i>. Cham: Springer, 2026. DOI: <a href=\"https://doi.org/10.1007/978-3-031-94229-7_225-1\" rel=\"noopener\" target=\"_blank\">10.1007/978-3-031-94229-7_225-1</a>.</p>" },
      ],
    ...
    es: {
      ...
      sections: [
        { __component: "case.text-section", body: "<p>Petrobras realiza un monitoreo continuo de las poblaciones de especies marinas a lo largo de sus áreas de operación, buscando comprender y mitigar eventuales impactos ambientales de sus actividades. Como parte de ese compromiso, la empresa es una de las mayores patrocinadoras de programas de protección de la biodiversidad marina en Brasil. Uno de los focos principales de ese esfuerzo es el ambiente acústico submarino: los cetáceos — ballenas y delfines — dependen del sonido para comunicarse, navegar y reproducirse, lo que los vuelve especialmente sensibles a los ruidos de origen antrópico, como los generados por campañas de prospección sísmica.</p><p>Para acompañar ese impacto, Petrobras cuenta con el Instituto Aqualie, una de las principales autoridades mundiales en bioacústica marina y responsable de uno de los programas de monitoreo acústico pasivo más longevos del Atlántico Sur. El Instituto combina distintas tecnologías — hidrófonos, tags acústicos, avistamientos visuales y seguimiento poblacional — para mapear la presencia, el comportamiento y la distribución estacional de cetáceos a lo largo de la costa brasileña.</p>" },
        { __component: "case.highlight-section", variant: "opening", eyebrow: "La solución", heading: "Fibra óptica como sensor acústico", body: "<p><b>Desde 2024, el Instituto Aqualie contrató a Immer Messen para desarrollar un enfoque inédito en Brasil:</b> transformar cables de telecomunicaciones submarinos ya instalados por Petrobras en sensores acústicos continuos, mediante la tecnología de Sensado Distribuido por Fibra Óptica (DFOS), específicamente en la modalidad DAS (Distributed Acoustic Sensing).</p>" },
        { __component: "case.text-section", body: "<p>El principio es directo: las fibras ópticas ociosas dentro de un cable de telecomunicaciones — que no transportan tráfico de datos — pueden ser interrogadas por un equipo instalado en tierra, capaz de detectar nanodeformaciones a lo largo de toda la extensión del cable. Cada metro de fibra se convierte, en la práctica, en un punto de escucha. No se requiere ninguna intervención física en la infraestructura submarina existente.</p>" },
        { figureKey: "mapa-campos" },
        { __component: "case.text-section", body: "<p>El proyecto también se apoya, de forma complementaria, en datos históricos de Permanent Reservoir Monitoring (PRM) — sensores sísmicos instalados originalmente por Petrobras en el Campo de Jubarte para monitoreo de reservorio — reaprovechados por el Instituto Aqualie para confirmar la ocurrencia estacional de ballenas jorobadas, ballenas fin y ballenas sei en la Cuenca de Campos.</p><p>Tras una fase de pruebas de laboratorio en 2024 — que validó la sensibilidad del sistema a sonidos de baja frecuencia y calibró parámetros como la tensión del cable y la longitud de gauge —, Immer Messen instaló su sistema de interrogación en una estación de telecomunicaciones en Barra do Furado (RJ), monitoreando un cable submarino de más de <b>110 km de extensión</b> en la Cuenca de Campos.</p>" },
        { __component: "case.section-title", title: "Inteligencia artificial en la detección de anomalías" },
        { __component: "case.two-column-section", leftBody: "<p>El volumen de datos generado por un sistema DAS operando continuamente sobre un centenar de kilómetros de cable es del orden de decenas de terabytes por campaña — inviable para inspección manual. Para manejar esa escala, Immer Messen desarrolla, desde 2020, un algoritmo propio de detección automática de anomalías, basado en el análisis de la densidad espectral de potencia (PSD) de la señal en cada punto del cable, a lo largo de ventanas temporales definidas. El algoritmo recorre el conjunto de datos en el espacio y en el tiempo, identificando variaciones abruptas de energía en bandas de frecuencia específicas y generando un catálogo de eventos — cada uno asociado a posición, horario y firma espectral — validado después por especialistas del Instituto Aqualie.</p>", pullQuote: "hoy, el monitoreo funciona en tiempo real", rightBody: "<p>Esa capa de inteligencia artificial es lo que vuelve el sistema operativamente viable: permite que los biólogos del Instituto Aqualie, desde su sede en Juiz de Fora, Minas Gerais, acompañen de forma remota la presencia de ballenas alrededor de los cables de telecomunicaciones de Petrobras — sin necesidad de embarcaciones, equipos en campo ni desplazamiento hasta el litoral.</p>" },
        { figureKey: "espectrograma" },
        { __component: "case.panel-section", icon: "bar_chart", defaultOpen: true, title: "Resultados", body: "<p>Las campañas de campo confirmaron la viabilidad de la tecnología para bioacústica a escala real. En una medición realizada a cerca de <b>21 km de la costa</b>, el sistema identificó 17 vocalizaciones distintas en una ventana de apenas 90 segundos, con patrones espectrales en la franja de <b>40 a 120 Hz</b> — consistentes con vocalizaciones de ballenas fin. Las señales aparecen en los datos como patrones característicos en forma de &quot;V&quot;, resultado de la propagación de la onda acústica a lo largo del cable, lo que permite no solo detectar el sonido sino también localizar su origen con precisión de pocos metros.</p>", blocks: [
          { heading: "Embarcaciones", body: "<p>La misma infraestructura y el mismo pipeline de detección demostraron sensibilidad a otras clases de señales relevantes para operaciones offshore. En el tramo de 30 a 37 km (a cerca de 33,5 km de la costa), el sistema registró patrones hiperbólicos secuenciales característicos de ruido de embarcación. La atenuación observada junto al ápice de cada patrón se relaciona con la geometría entre el movimiento de la embarcación y el cable: la intensidad aumenta a medida que la embarcación se aproxima a la fibra y disminuye cuando se aleja. La evolución temporal de esas llegadas permite inferir la dirección de movimiento y, integrada a datos de AIS, estimar la trayectoria de la embarcación.</p>", figureKey: "embarcacoes" },
          { heading: "Procesos oceánicos y zona de rompiente", body: "<p>En la sección costera, donde el cable entra en el mar, el mismo conjunto de datos capturó con claridad procesos oceánicos de gran escala. Con filtrado pasa-alta en 0,01 Hz, se evidenciaron patrones lentos y periódicos propagándose del mar hacia la costa, atribuidos a la modulación de la presión hidrostática por ondas de gravedad de superficie sobre la fibra enterrada. Ajustando el corte a 5 Hz, esos patrones terminan abruptamente en una transición espacial específica, a partir de la cual emergen oscilaciones mecánicas de más alta frecuencia asociadas a la dinámica de rompiente en la interfaz arena–mar. Esa respuesta espectral y espacialmente amplia — de procesos oceánicos de baja frecuencia a emisiones acústicas de más alta frecuencia — evidencia la versatilidad del DAS como sensor ambiental.</p>", figureKey: "processos-oceanicos" },
          { body: "<p>Esa característica es central para el valor de la solución — la especificidad de la detección está en el software, no en el hardware, lo que permite calibrar el mismo sistema físico para distintas finalidades de monitoreo.</p>" },
        ] },
        { __component: "case.section-title", title: "Impacto y próximos pasos" },
        { __component: "case.text-section", body: "<p>Los resultados deben contribuir directamente al entendimiento científico de los patrones migratorios de cetáceos en el litoral brasileño. Especies como la ballena jorobada utilizan el corredor entre el Atlántico Sur y el litoral nordestino durante la temporada reproductiva, y datos continuos como los generados por el sistema DAS ayudarán al Instituto Aqualie a refinar estimaciones poblacionales e identificar qué especies transitan por distintos tramos de la costa a lo largo del año — información esencial para orientar la planificación de campañas sísmicas con menor impacto ambiental.</p><p>La alianza entre Immer Messen, Instituto Aqualie y Petrobras sigue en desarrollo, con el objetivo de expandir la cobertura geográfica del monitoreo y perfeccionar los modelos de clasificación automática por especie a lo largo de los próximos años.</p>" },
        { __component: "case.highlight-section", variant: "closing", heading: "El proyecto es reconocido como el primer monitoreo de ballenas por DAS realizado en el Atlántico Sur", body: "<p>Esto consolida a Immer Messen como <b>referencia en soluciones de monitoreo acústico</b> distribuido para ambientes offshore — una capacidad que se extiende, con la misma base tecnológica, a aplicaciones como seguridad de activos submarinos, detección de intrusión en ductos y cables, y monitoreo estructural de infraestructura crítica.</p>" },
        { __component: "case.text-section", body: "<p>Los resultados descritos en este caso están documentados en un capítulo de libro revisado por pares, de acceso abierto:</p><p>ANDRIOLO, A.; MARCON, E. H.; DE CASTRO, F. R.; RODRIGUES, G. M.; MIRANDA, G. A.; VIANA, Y.; GOLODNE, P. M.; POEYS, R. A. C.; SILVA, A. A. C.; AMORIM, T. O. S.; DJOKIC, D.; PIZZORNO, J. L. A. Permanent Reservoir Monitoring (PRM) and Distributed Acoustic Sensing (DAS) as Advanced Technologies for Maximizing the Acquisition of Acoustic Environmental Information. In: POPPER, A. N. et al. (eds.). <i>The Effects of Noise on Aquatic Life IV</i>. Cham: Springer, 2026. DOI: <a href=\"https://doi.org/10.1007/978-3-031-94229-7_225-1\" rel=\"noopener\" target=\"_blank\">10.1007/978-3-031-94229-7_225-1</a>.</p>" },
      ],
  ...
  },
  {
    ...
    en: {
      ...
      sections: [
        { __component: "case.highlight-section", variant: "opening", eyebrow: "The solution", heading: "Maximum precision without complex infrastructure", body: "<p>A major Brazilian natural gas transmission company contracted Immer Messen for a project of continuous, autonomous monitoring of a pipeline section using DFOS — Distributed Fibre Optic Sensing — technology combined with Artificial Intelligence.</p>" },
        { __component: "case.text-section", body: "<p>Using an ordinary telecom optical fibre cable installed along the pipeline, the Immer Messen DATS system turned the fibre into a dense array of distributed acoustic sensors, able to detect any disturbance along the entire monitored length — with no additional equipment in the field, no continuous human intervention and no complex infrastructure works.</p>" },
        { __component: "case.section-title", title: "How it works" },
        { __component: "case.text-section", body: "<p>The DATS optical interrogator sends light pulses along the fibre and analyses the reflected signals with a spatial resolution of 3 metres. Any acoustic event or mechanical vibration along the pipeline — a human footstep, a passing vehicle or the vibration of an excavator — alters the signal pattern in a characteristic way. <b>The Immer Messen AI algorithms process these variations in real time and automatically classify every detected event.</b></p><p>The operator follows everything remotely, from a control room more than 200 km away from the monitored section.</p>" },
        { __component: "case.panel-section", icon: "bar_chart", defaultOpen: true, title: "Results", body: "<p>Over weeks of autonomous, continuous operation, the DATS system performed strongly across every scenario tested. Blind simulated events were staged along the monitored section, with the operations team unaware of the type, location or timing of each occurrence. The system detects the anomalies and classifies them into the following event classes:</p><ul><li>Person walking along the right of way (signal example I.)</li><li>Motorcycle</li><li>Light vehicle (signal example II.)</li><li>Heavy vehicle</li><li>Excavator in operation (signal example III.)</li><li>Pipeline drilling</li><li>Animals (cattle moving in the adjacent pasture)</li></ul><p><b>Every alert was triggered and sent by SMS to the operations team in under 1 minute after the event — with no local supervision whatsoever.</b></p>", blocks: [
          { body: "<p>Some samples of detection signals and automatic classification are shown below.</p><p>I. Person walking along the right of way: explicit signals reveal the footsteps and the movement of the person in the vicinity of the optical cable;</p><p>II. Light vehicle: more intense mechanical patterns spreading through space-time, whose apexes reveal spatial displacement at higher speed and under varying intensities — associated with the uneven terrain of the region;</p><p>III. Excavator: the signal shows three types of pattern that reveal distinct stages of operation. At the start, the signals are associated with actual digging, the periodic action creating intense mechanical patterns that spread through space. In the second stage there is steady behaviour with dominant higher-frequency patterns, associated only with the excavator switched on but stationary. In the third stage, more intense mechanical signals with spatial-temporal displacement reveal the excavator moving.</p>", figureKey: "dutos-sinais" },
        ] },
        { __component: "case.section-title", title: "Technical differentiators" },
        { __component: "case.text-section", body: "<ul><li>Continuous 24/7 monitoring, with no supervision in the field</li><li>Automatic AI event classification — not just detection, but identification of the type of threat</li><li>Real-time SMS alerts in under 1 minute</li><li>Remote monitoring from a control room hundreds of kilometres from the monitored point</li><li>Operation over ordinary telecom optical fibre — with no additional point sensors</li><li>Spatial resolution of 3 metres along the entire monitored section</li></ul>" },
        { figureKey: "dutos-dashboard" },
        { __component: "case.highlight-section", variant: "closing", heading: "Conclusion: DFOS technology and AI raise pipeline security to a new level", body: "<p>The project showed that Immer Messen DFOS technology, combined with proprietary AI algorithms, can turn the optical fibre already available in pipeline infrastructure into a robust, autonomous and precise security monitoring system — <b>giving the operator real-time visibility that conventional methods simply do not offer.</b></p>" },
      ],
    ...
    es: {
      ...
      sections: [
        { __component: "case.highlight-section", variant: "opening", eyebrow: "La solución", heading: "Máxima precisión sin infraestructura compleja", body: "<p>Una gran transportadora de gas natural de Brasil contrató a Immer Messen para un proyecto de monitoreo continuo y autónomo de un tramo de gasoducto mediante tecnología DFOS — Distributed Fiber Optic Sensing — combinada con Inteligencia Artificial.</p>" },
        { __component: "case.text-section", body: "<p>Utilizando un cable de fibra óptica de telecomunicaciones común instalado a lo largo del ducto, el sistema DATS de Immer Messen transformó la fibra en una red densa de sensores acústicos distribuidos, capaz de detectar cualquier perturbación a lo largo de toda la extensión monitoreada — sin equipos adicionales en campo, sin intervención humana continua y sin obras de infraestructura complejas.</p>" },
        { __component: "case.section-title", title: "Cómo funciona" },
        { __component: "case.text-section", body: "<p>El interrogador óptico DATS envía pulsos de luz por la fibra y analiza las señales reflejadas con resolución espacial de 3 metros. Cualquier evento acústico o vibración mecánica a lo largo del ducto — un paso humano, el paso de un vehículo o la vibración de una excavadora — altera el patrón de la señal de forma característica. <b>Los algoritmos de IA de Immer Messen procesan esas variaciones en tiempo real y clasifican automáticamente cada evento detectado.</b></p><p>El operador acompaña todo de forma remota, desde su sala de control, a más de 200 km de distancia del tramo monitoreado.</p>" },
        { __component: "case.panel-section", icon: "bar_chart", defaultOpen: true, title: "Resultados", body: "<p>Durante semanas de operación autónoma y continua, el sistema DATS demostró un desempeño excelente en todos los escenarios probados. Se realizaron eventos simulados a ciegas a lo largo del tramo monitoreado, sin que el equipo de operación supiera el tipo, el lugar ni el momento de cada ocurrencia. El sistema detecta las anomalías y las clasifica en las siguientes clases de evento:</p><ul><li>Persona caminando por la franja (ejemplo de señal I.)</li><li>Motocicleta</li><li>Vehículo ligero (ejemplo de señal II.)</li><li>Vehículo pesado</li><li>Excavadora en operación (ejemplo de señal III.)</li><li>Perforación del ducto</li><li>Animales (ganado transitando por el pasto adyacente)</li></ul><p><b>Todas las alertas fueron disparadas y enviadas por SMS al equipo de operación en menos de 1 minuto tras el evento — sin ninguna supervisión local.</b></p>", blocks: [
          { body: "<p>A continuación se presentan algunas muestras de señales de detección y clasificación automática.</p><p>I. Persona caminando por la franja: señales explícitas revelan los pasos y el desplazamiento de la persona en las inmediaciones del cable óptico;</p><p>II. Vehículo ligero: patrones mecánicos más intensos que se propagan por el espacio-tiempo, cuyos ápices revelan el desplazamiento espacial con velocidad superior y bajo intensidades variables — asociadas al relieve irregular de la región;</p><p>III. Excavadora: la señal presenta tres tipos de patrón que revelan etapas de operación distintas. Al inicio las señales se asocian a excavaciones propiamente dichas, con la acción periódica creando patrones mecánicos intensos que se propagan por el espacio. En la segunda etapa se observa un comportamiento constante con patrones de más alta frecuencia dominantes, asociados solamente a la máquina encendida pero detenida. En la tercera etapa, señales mecánicas más intensas con desplazamiento espacio-temporal revelan el movimiento de la excavadora.</p>", figureKey: "dutos-sinais" },
        ] },
        { __component: "case.section-title", title: "Diferenciales técnicos" },
        { __component: "case.text-section", body: "<ul><li>Monitoreo continuo 24/7, sin supervisión en campo</li><li>Clasificación automática de eventos por IA — no solo detección, sino identificación del tipo de amenaza</li><li>Alertas en tiempo real vía SMS en menos de 1 minuto</li><li>Acompañamiento remoto en sala de control a cientos de kilómetros del punto monitoreado</li><li>Operación sobre fibra óptica de telecomunicaciones común — sin sensores puntuales adicionales</li><li>Resolución espacial de 3 metros a lo largo de todo el tramo monitoreado</li></ul>" },
        { figureKey: "dutos-dashboard" },
        { __component: "case.highlight-section", variant: "closing", heading: "Conclusión: la tecnología DFOS y la IA elevan la seguridad de ductos a un nuevo nivel", body: "<p>El proyecto demostró que la tecnología DFOS de Immer Messen, combinada con algoritmos propietarios de IA, es capaz de transformar la fibra óptica ya disponible en infraestructuras de ductos en un sistema robusto, autónomo y preciso de monitoreo de seguridad — <b>entregando al operador una visibilidad en tiempo real que los métodos convencionales simplemente no ofrecen.</b></p>" },
      ],
...
const validateSeed = () => {
  ...
  // Sao 2 cases desde a reducao do 8d9d057 (so os dois PDFs aprovados pelo
  // cliente); a guarda antiga ainda exigia os 4 do conteudo mock.
  if (seedContent.caseStudies.length < caseStudyDefs.length * locales.length) {
    errors.push(
      `Expected ${caseStudyDefs.length * locales.length} localized case studies, found ${seedContent.caseStudies.length}`
    );
  }
